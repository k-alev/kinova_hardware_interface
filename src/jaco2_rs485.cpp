...
const float Jaco2::MAX_TORQUE[6] = {40.0, 40.0, 40.0, 20.0, 20.0, 20.0};  // in Nm //KA_changed 2nd joint
const unsigned char Jaco2::JOINT_ADDRESS[6] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15};
const unsigned int Jaco2::TORQUE_DAMPING[6] = {4, 2, 1, 2, 1, 1};
const short Jaco2::TORQUE_KP[6] = {1000, 1000, 1000, 1750, 1750, 1750}; //KA_changed 2nd joint
const float Jaco2::STATIC_FRICTION[6] = {0.5f, 0.5f, 0.5f, 2.0f, 6.0f, 9.0f};//{0.5f, 0.5f, 0.5f, 2.0f, 7.0f, 10.0f}; //KA_addition
...
Jaco2::Jaco2(int a_display_error_level) {
    ...
    fptrCloseCommunication = (int (*)()) dlsym(commLayer_Handle,"CloseCommunication");
    ...
    for (int ii=0; ii<6; ii++) {
        ...
        torques_config_feedforward_advanced_message[ii].DataFloat[2] = STATIC_FRICTION[ii];  // static friction;
        torques_config_feedforward_advanced_message[ii].DataFloat[3] = 5.0f;//10.0;  // max static friction;
    }
...
void Jaco2::WriteForces(float u[6])
{
    // load the torque signal into outbound message
    for (int ii=0; ii<6; ii++) {
        force_message[ii].DataFloat[0] = pos[ii];
    ...
    int k;
    k = MyRS485_Write(force_message, packets_sent, write_count);
    //std::cout << "wrote:"<<k<< std::endl;
}


void Jaco2::ReadForces()
{
    int k;
    updated_sum = 0;
    memset(updated, 0, (size_t)sizeof(int)*6);

    k = MyRS485_Read(feedback_message, packets_read, read_count);
